...
void assign_area( CHAR_DATA * ch )
{
   ...
   if( get_trust( ch ) > LEVEL_IMMORTAL && ch->pcdata->r_range_lo && ch->pcdata->r_range_hi )
   {
      ...
      snprintf( taf, 1024, "%s.zona", capitalize( ch->name ) );
      if( !tarea )
      {
...
void do_aassign( CHAR_DATA * ch, char *argument )
{
   ...
   if( argument[0] == '\0' )
   {
      send_to_char( "Sintaxis: aasignar <fichero.zona>\r\n", ch );
      return;
   }
 ...
 *   (a) already installed but removed from zona.lista
 *   (b) designed offline
...
void do_installarea( CHAR_DATA * ch, char *argument )
{
   ...
   for( tarea = first_build; tarea; tarea = tarea->next )
   {
      if( !str_cmp( tarea->filename, arg ) )
      {
         ...
         top_area++;
         send_to_char( "Writing zona.lista ...\r\n", ch );
         write_area_list(  );
