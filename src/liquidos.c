...
/* save the liquids to the liquidos.dat file in the system directory -Nopey */
void save_liquids( void )
{
   ...
   snprintf( filename, 256, "%sliquidos.datos", SYSTEM_DIR );
...
void save_mixtures( void )
{
   ...
   snprintf( filename, 256, "%smezclas.datos", SYSTEM_DIR );
   if( !( fp = fopen( filename, "w" ) ) )
   {
      bug( "%s: cannot open %s for writing", __FUNCTION__, filename );
      return;
