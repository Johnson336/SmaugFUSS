...
void do_hset( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg1, "save" ) )
   {
      ...
      log_string_plus( "Guardando ayuda.zona ...", LOG_NORMAL, LEVEL_GREATER );

      rename( "ayuda.zona", "ayuda.zona.bak" );
      if( ( fpout = fopen( "ayuda.zona", "w" ) ) == NULL )
      {
         bug( "%s: cant open ayuda.zona", __FUNCTION__ );
         perror( "ayuda.zona" );
         return;
      }
