...
char *const sect_names[SECT_MAX][2] = {
   {"En una habitacion", "inside"}, {"En una ciudad", "cities"},
   {"En un campo", "fields"}, {"En un bosque", "forests"},
   {"colina", "hills"}, {"En una montanha", "mountains"},
   {"En el agua", "waters"}, {"En aguas peligrosas", "waters"},
   {"Bajo el agua", "underwaters"}, {"En el aire", "air"},
   {"En un desierto", "deserts"}, {"En algun lugar", "unknown"},
   {"en el fondo oceanico", "ocean floor"}, {"bajo tierra", "underground"}
};
...
ch_ret move_char( CHAR_DATA * ch, EXIT_DATA * pexit, int fall )
{
   ...
   if( !pexit || ( to_room = pexit->to_room ) == NULL )
   {
          ...
          && ch->in_room->sector_type != SECT_UNDERWATER && ch->in_room->sector_type != SECT_OCEANFLOOR )
      {
         ...
         send_to_char( "Mira a tu alrededor, no puedes ir en esa direccion.\r\n", ch );
      return rNONE;
   }
   ...
   {
      send_to_char( "Como te atreves? Y abandonar a tu ansiado dios?\r\n", ch );
      return rNONE;
   }

   if( room_is_private( to_room ) )
   {
      send_to_char( "Esa habitacion es privada por ahora.\r\n", ch );
      return rNONE;
   }
