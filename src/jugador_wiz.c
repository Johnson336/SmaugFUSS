...
void do_balzhur( CHAR_DATA * ch, char *argument )
{
   ...
   snprintf( buf2, MAX_STRING_LENGTH, "%s.zona", name );
...
void do_mortalize( CHAR_DATA * ch, char *argument )
{
   ...
   if( stat( fname, &fst ) != -1 )
   {
      ...
      snprintf( buf2, MAX_STRING_LENGTH, "%s.zona", capitalize( argument ) );
      for( pArea = first_build; pArea; pArea = pArea->next )
...
void do_newbieset( CHAR_DATA * ch, char *argument )
{
    ...
    * guide to the realms of despair, part of nueva_academia.zona. 
    */
   {
...
/* Added the burlap sack to the newbieset.  The sack is part of nuevo_portal.zona
 ...
 * e.g. "aset joe.zona sedit susan.zona cset" --> "joe.zona susan.zona"
 * - Gorog
 */
void extract_area_names( char *inp, char *out )
{
   ...
   *out = '\0';
   while( inp && *inp )
   {
      ...
      if( ( len = strlen( buf ) ) >= 5 && !strcmp( ".zona", pbuf + len - 4 ) )
      {
         if( *out )
 ...
 * e.g. "aset joe.zona sedit susan.zona cset" --> "aset sedit cset"
 * - Gorog
 */
...
/*
 * Allows members of the Zona Feudo to add Zona names to the bestow field.
 * Area names mus end with ".zona" so that no commands can be bestowed.
 */
void do_bestowarea( CHAR_DATA * ch, char *argument )
{
   ...
   {
      send_to_char( "Syntax:\r\n"
                    "bestowarea <victim> <filename>.zona\r\n"
   ...
   {
      send_to_char( "You can only bestow an zona name\r\n", ch );
      send_to_char( "E.G. bestow joe sam.zona\r\n", ch );
      return;
   }
...
void do_setrace( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg2, "create" ) )
   {
      ...
      {
         send_to_char( "You need to up MAX_RACE in phantasienIII.h and make clean.\r\n", ch );
         return;
      }
...
void do_pcrename( CHAR_DATA * ch, char *argument )
{
   ...
   if( victim->pcdata->area )
   {
      ...
      snprintf( filename, MAX_STRING_LENGTH, "%s%s.zona", BUILD_DIR, victim->name );
      snprintf( newfilename, MAX_STRING_LENGTH, "%s%s.zona", BUILD_DIR, capitalize( arg2 ) );
      rename( filename, newfilename );
      snprintf( filename, MAX_STRING_LENGTH, "%s%s.zona.bak", BUILD_DIR, victim->name );
      snprintf( newfilename, MAX_STRING_LENGTH, "%s%s.zona.bak", BUILD_DIR, capitalize( arg2 ) );
      rename( filename, newfilename );
   }
