...
void cleanup_memory( void )
{
...
#ifdef IMC
   fprintf( stdout, "%s", "Datos de IMC2.\n" );
   free_imcdata( TRUE );
   imc_delete_info(  );
#endif

   fprintf( stdout, "%s", "Datos de Proxectos.\n" );
   free_projects(  );

   fprintf( stdout, "%s", "Nomes reservados.\n" );
   free_all_reserved(  );

   fprintf( stdout, "%s", "Datos de Baneados.\n" );
   free_bans(  );

   fprintf( stdout, "%s", "Datos de Polimorfismos.\n" );
   free_morphs(  );
    ...
    * Councils
    */
   fprintf( stdout, "%s", "Comarcas.\n" );
   free_councils(  );

   /*
    * socials 
    */
   fprintf( stdout, "%s", "Sociais.\n" );
   free_socials(  );

   /*
    * Watches 
    */
   fprintf( stdout, "%s", "Vistas.\n" );
   free_watchlist(  );

   /*
    * Helps 
    */
   fprintf( stdout, "%s", "Axudas.\n" );
   free_helps(  );

   /*
    * Languages 
    */
   fprintf( stdout, "%s", "Linguas.\n" );
   free_tongues(  );

   /*
    * Boards 
    */
   fprintf( stdout, "%s", "Taboeiros.\n" );
   free_boards(  );
    ...
    * Liquids 
    */
   fprintf( stdout, "%s", "Taboa de Liquidos.\n" );
   free_liquiddata(  );

   /*
    * Races 
    */
   fprintf( stdout, "%s", "Razas.\n" );
    ...
    * Teleport lists 
    */
   fprintf( stdout, "%s", "Datos de Teleportes.\n" );
   free_teleports(  );

   /*
    * Areas - this includes killing off the hash tables and such 
    */
   fprintf( stdout, "%s", "Taboas de datos de Zonas.\n" );
   close_all_areas(  );

   /*
    * Planes
    */
   fprintf( stdout, "%s", "Planos.\n" );
   free_all_planes(  );

   /*
    * Get rid of auction pointer  MUST BE AFTER OBJECTS DESTROYED 
    */
   fprintf( stdout, "%s", "Subasta.\n" );
   DISPOSE( auction );

   /*
    * System Data 
    */
   fprintf( stdout, "%s", "Datos do Sistema.\n" );
    ...
    * Title table 
    */
   fprintf( stdout, "%s", "Taboa de titulos.\n" );
   for( hash = 0; hash < MAX_CLASS; hash++ )
   {
    ...
    * Skills 
    */
   fprintf( stdout, "%s", "Feitizos e herbas.\n" );
   free_skills(  );

   /*
    * Prog Act lists 
    */
   fprintf( stdout, "%s", "Listado de Actores Mudprog.\n" );
   free_prog_actlists(  );
   ...
   fprintf( stdout, "%s", "Limpeza completa, saindo.\n" );
   return;
}  /* cleanup memory */
...
#endif
{
   ...
   if( argc > 1 )
   {
      ...
      {
         fprintf( stderr, "O porto debe estar por encima  de 1024.\n" );
         exit( 1 );
      }
      if( argv[2] && argv[2][0] )
      {
   ...
   log_string( "Iniciando base de datos ..." );
   boot_db( fCopyOver );
   log_string( "Iniciando socket ..." );
   ...
   log_printf( "%s activo no porto %d.", sysdata.mud_name, port );

   if( fCopyOver )
   {
      log_string( "Cargando a recuperacion do inicio rapido." );
      hotboot_recover(  );
   }
    ...
    * That's all, folks.
    */
   log_string( "Terminacion normal do xogo." );

   log_string( "Limpiando memoria." );
   cleanup_memory(  );
...
void nanny_read_motd( DESCRIPTOR_DATA * d, char *argument )
{
   ...
   if( ch->level == 0 )
   {
       ...
       * to the realms of despair, part of nueva_academia.zona.
       */
      {
...
void display_prompt( DESCRIPTOR_DATA * d )
{
   ...
   const char *prompt;
   const char *helpstart = "<Teclea AXUDA START>";
   char buf[MAX_STRING_LENGTH];
   char *pbuf = buf;
   unsigned int pstat;
   int percent;
...
bool pager_output( DESCRIPTOR_DATA * d )
{
   ...
   if( xIS_SET( ch->act, PLR_ANSI ) )
      if( write_to_descriptor( d, ANSI_LBLUE, 0 ) == FALSE )
         return FALSE;
   if( ( ret = write_to_descriptor( d, "(C)ontinua, (N)on-parar, (R)efrescar, (A)trais, (S)air: [C] ", 0 ) ) == FALSE )
      return FALSE;
   if( xIS_SET( ch->act, PLR_ANSI ) )
   {
      char buf[32];
