...
void do_hset( CHAR_DATA * ch, char *argument )
{
   ...
   if( !str_cmp( arg1, "save" ) )
   {
      ...
      log_string_plus( "Gardando axuda.zona ...", LOG_NORMAL, LEVEL_GREATER );

      rename( "axuda.zona", "axuda.zona.bak" );
      if( ( fpout = fopen( "axuda.zona", "w" ) ) == NULL )
      {
         bug( "%s: Non se pode abrir axuda.zona", __FUNCTION__ );
         perror( "axuda.zona" );
         return;
      }

      fprintf( fpout, "#AXUDAS\n\n" );
      for( pHelp = first_help; pHelp; pHelp = pHelp->next )
...
void do_version( CHAR_DATA * ch, char *argument )
{
   ...
   set_char_color( AT_YELLOW, ch );
   ch_printf( ch, "Kallaikia (SMAUG) %s.%s.%s\r\n", SMAUG_VERSION_MAJOR, SMAUG_VERSION_MINOR, SMAUG_VERSION_KALLAIKIA );
