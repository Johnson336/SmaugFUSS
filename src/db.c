...
void boot_db( bool fCopyOver )
{
   ...
   show_hash( 32 );
   unlink( BOOTLOG_FILE );
   boot_log( "---------------------[ Log de inicio ]--------------------" );

   log_string( "Inicializando soporte para libdl ..." );
   /*
   ...
   if( !sysdata.dlHandle )
   {
      log_string( "Enlazador dinamico: Error abriendo el ejecutable, revisa los FLAGS utilizados en Makefile." );
   ...
   log_string( "Cargando spec_funs ..." );
   load_specfuns(  );

   log_string( "Cargando configuracion del sistema ..." );
   ...
   {
      log_string( "No encontrado.  Creando nueva configuracion." );
      sysdata.alltimemax = 0;
      sysdata.mud_name = str_dup( "(Nombre no configurado)" );
      update_timers(  );
      update_calendar(  );
      save_sysdata( sysdata );
   }

   log_string( "Cargando habilidades sociales ... " );
   load_socials(  );

   log_string( "Cargando tabla de hechidos y habilidades ... " );
   load_skill_table(  );
   sort_skill_table(  );
   ...
   log_string( "Cargando datos de noticias" );
   load_news(  );

   /*
    * load liquids into the system from the liquidos.datos file -Nopey 
    */
   log_string( "Cargando liquidos" );
   load_liquids(  );

   /*
    * load mixtures into the system from the mixturetable.dat file -Nopey 
    */
   log_string( "Cargando mezclas" );
   load_mixtures(  );

   log_string( "Cargando tabla de hierbas ..." );
   load_herb_table(  );

   log_string( "Cargando idiomas ..." );
   load_tongues(  );
    ...
    */
   log_string( "Inicializando generador de numeros aleatorios ..." );
   init_mm(  );
    ...
    */
   {
      ...
      log_string( "Estableciendo fecha y climatologia." );

      if( !load_timedata(  ) )   /* Loads time from stored file if TRUE - Samson 1-21-99 */
      {
         boot_log( "Restaurando la fecha basandose en la actual del sistema." );
   ...
   log_string( "Cargando tabla de vacaciones ..." ); /* Samson 5-13-99 */
   load_holidays(  );

   log_string( "Cargando  DNS cache..." );  /* Samson 1-30-02 */
   load_dns(  );
...
#ifdef PLANES
   log_string( "Analizando fichero con datos de planos ..." );
   load_planes(  );
#endif
    ...
    */
   {
      ...
      log_string( "Analizando ficheros con datos de zonas del reino ..." );
      if( !( fpList = fopen( AREA_LIST, "r" ) ) )
      {
         perror( AREA_LIST );
         shutdown_mud( "ERROR! No se puede abrir zonas.lista" );
         exit( 1 );
      }
      ...
      for( ;; )
      {
         if( feof( fpList ) )
         {
            bug( "%s: EOF encontrado analizando %s no se ha encontrado $ al final del fichero.", __FUNCTION__, AREA_LIST );
            break;
         }
...
#ifdef PLANES
   log_string( "Comprobando que se ha generado el plano de las habitaciones ..." );
   check_planes( NULL );
#endif
   ...
   log_string( "Estableciendo salidas" );
   fix_exits(  );
   fBootDb = FALSE;
   log_string( "Inicializando economia" );
   initialize_economy(  );
   if( fCopyOver )
   {
      log_string( "Cargando el estado del mundo ..." );
      load_world(  );
   }
   log_string( "Reiniciando zonas ..." );
   area_update(  );

   log_string( "Cargando lista de edificaciones ... " );
   load_buildlist(  );

   log_string( "Cargando tableros de anuncios ... " );
   load_boards(  );

   log_string( "Cargando clanes" );
   load_clans(  );

   log_string( "Cargando feudos" );
   load_councils(  );

   log_string( "Cargando deidades" );
   load_deity(  );

   log_string( "Cargando vistas" );
   load_watchlist(  );

   log_string( "Cargando usuarios baneados" );
   load_banlist(  );

   log_string( "Cargando nombres reservados" );
   load_reserved(  );

   log_string( "Cargando cadaveres" );
   load_corpses(  );

   log_string( "Cargando servidores Inmortales ..." );
   load_imm_host(  );

   log_string( "Cargando proyectos" );
   load_projects(  );

   /*
    * Morphs MUST be loaded after class and race tables are set up --Shaddai 
    */
   log_string( "Cargando polimorfismos/mutaciones" );
   load_morphs(  );
   MOBtrigger = TRUE;
...
void add_help( HELP_DATA * pHelp )
{
   ...
   for( tHelp = first_help; tHelp; tHelp = tHelp->next )
      ...
      {
         bug( "add_help: duplicado: %s.  Eliminando.", pHelp->keyword );
         STRFREE( pHelp->text );
...
void load_mobiles( AREA_DATA * tarea, FILE * fp )
{
   ...
   for( ;; )
   {
      ...
      if( letter != '#' )
      {
         bug( "Cargar criaturas (Load_mobiles): # no encontrado." );
         if( fBootDb )
         {
      ...
      if( get_mob_index( vnum ) )
      {
         if( tmpBootDb )
         {
            bug( "Cargar criaturas (Load_mobiles): vnum %d duplicado.", vnum );
         ...
         else
         {
            ...
            log_printf_plus( LOG_BUILD, sysdata.log_level, "Limpiando criaturas mobile: %d", vnum );
            clean_mob( pMobIndex );
            oldmob = TRUE;
         }
      }
      ...
      {
         bug( "Cargar criaturas (Load_mobiles): vnum %d: la letra '%c' no es S o C.", vnum, letter );
         ...
         else  /* SmaugWiz zone here */
         {
            ...
            else
            {
               while( ln[0] != '\0' )
               {
                  ...
                  if( value == -1 )
                     bug( "No conocido, hablar idioma: %s\r\n", flag );
                  else
...
void load_objects( AREA_DATA * tarea, FILE * fp )
{
   ...
   for( ;; )
   {
      ...
      if( get_obj_index( vnum ) )
      {
         if( tmpBootDb )
         {
            bug( "Cargar objetos: vnum %d duplicado.", vnum );
            shutdown_mud( "duplicate vnum" );
            exit( 1 );
         }
         else
         {
            ...
            log_printf_plus( LOG_BUILD, sysdata.log_level, "Limpiando objeto: %d", vnum );
            clean_obj( pObjIndex );
            oldobj = TRUE;
         }
      }
...
void load_resets( AREA_DATA * tarea, FILE * fp )
{
   ...
   if( !tarea->first_room )
   {
      bug( "%s: No se ha encontrado la seccion #ROOMS. No se pueden cargar los reinicios.", __FUNCTION__ );
      if( fBootDb )
      {
   ...
   for( ;; )
   {
      ...
      switch ( letter )
      {
         default:
            ...
            if( fBootDb )
               boot_log( "%s: %s (%d) comando equivocado '%c'.", __FUNCTION__, tarea->filename, count, letter );
            return;

         case 'M':
            if( get_mob_index( arg1 ) == NULL && fBootDb )
               boot_log( "%s: %s (%d) 'M': mobile %d no existe.", __FUNCTION__, tarea->filename, count, arg1 );

            if( ( pRoomIndex = get_room_index( arg3 ) ) == NULL && fBootDb )
               boot_log( "%s: %s (%d) 'M': room %d no existe.", __FUNCTION__, tarea->filename, count, arg3 );
            else
         ...
         case 'O':
            if( get_obj_index( arg1 ) == NULL && fBootDb )
               boot_log( "%s: %s (%d) '%c': el objecto %d no existe.", __FUNCTION__, tarea->filename, count, letter, arg1 );

            if( ( pRoomIndex = get_room_index( arg3 ) ) == NULL && fBootDb )
               boot_log( "%s: %s (%d) '%c': la habitacion %d no existe.", __FUNCTION__, tarea->filename, count, letter, arg3 );
            else
            {
               if( !pRoomIndex )
                  bug( "%s: No se puede añadir el reinicio - habitacion no encontrada.", __FUNCTION__ );
               else
                  add_reset( pRoomIndex, letter, extra, arg1, arg2, arg3 );
            }
            break;
         ...
         case 'P':
            if( get_obj_index( arg1 ) == NULL && fBootDb )
               boot_log( "%s: %s (%d) '%c': el objet %d no existe.", __FUNCTION__, tarea->filename, count, letter, arg1 );
            if( arg3 > 0 )
            {
               if( get_obj_index( arg3 ) == NULL && fBootDb )
                  boot_log( "%s: %s (%d) 'P': el objeto destino %d no existe.", __FUNCTION__, tarea->filename, count,
                            arg3 );
               if( extra > 1 )
                  not01 = TRUE;
            }
            if( !pRoomIndex )
               bug( "%s: No se ha podido añadir el reinicio - habitacion no encontrada.", __FUNCTION__ );
            else
            {
               if( arg3 == 0 )
         ...
         case 'E':
            if( get_obj_index( arg1 ) == NULL && fBootDb )
               boot_log( "%s: %s (%d) '%c': el objeto %d no existe.", __FUNCTION__, tarea->filename, count, letter, arg1 );
            if( !pRoomIndex )
               bug( "%s: No se ha podido añadir el reinicio - habitacion no encontrada.", __FUNCTION__ );
            else
               add_reset( pRoomIndex, letter, extra, arg1, arg2, arg3 );
         ...
         case 'H':
            bug( "%s: No se puede convertir el formato antiguo de los reinicios ocultos. Debe convertirse manualmente.", __FUNCTION__ );
            break;

         case 'D':
            if( !( pRoomIndex = get_room_index( arg1 ) ) )
            {
               bug( "%s: 'D': la habitacion %d no existe.", __FUNCTION__, arg1 );
               bug( "Reinicio: %c %d %d %d %d", letter, extra, arg1, arg2, arg3 );
               if( fBootDb )
                  boot_log( "%s: %s (%d) 'D': la habitacion %d no existe.", __FUNCTION__, tarea->filename, count, arg1 );
               break;
            }
            ...
            {
               bug( "%s: 'D': salida %d sin puerta.", __FUNCTION__, arg2 );
               bug( "Reset: %c %d %d %d %d", letter, extra, arg1, arg2, arg3 );
               if( fBootDb )
                  boot_log( "%s: %s (%d) 'D': salida %d sin puerta.", __FUNCTION__, tarea->filename, count, arg2 );
            }
         ...
         case 'R':
            if( !( pRoomIndex = get_room_index( arg1 ) ) && fBootDb )
               boot_log( "%s: %s (%d) 'R': la habitacion %d no existe.", __FUNCTION__, tarea->filename, count, arg1 );
            else
               add_reset( pRoomIndex, letter, extra, arg1, arg2, arg3 );
            if( arg2 < 0 || arg2 > 10 )
            {
               ...
               if( fBootDb )
                  boot_log( "%s: %s (%d) 'R': %d no es una salida.", __FUNCTION__, tarea->filename, count, arg2 );
               break;
            }
            break;
      }
   ...
   switch ( letter2 )
   {
      ...
      case 'M':
         if( get_mob_index( arg2 ) == NULL && fBootDb )
            boot_log( "%s: SmaugWiz - %s (%d) 'M': mobile %d no existe.", __FUNCTION__, room->area->filename, count,
                      arg2 );
         break;

      case 'O':
         if( get_obj_index( arg2 ) == NULL && fBootDb )
            boot_log( "%s: SmaugWiz - %s (%d) '%c': object %d no existe.", __FUNCTION__, room->area->filename, count,
                      letter2, arg2 );
         break;
      ...
      case 'P':
         ...
         if( arg4 > 0 )
         {
            if( get_obj_index( arg4 ) == NULL && fBootDb )
               boot_log( "$s: SmaugWiz - %s (%d) 'P': destination object %d no existe.", __FUNCTION__,
      ...
      case 'H':
         if( arg1 > 0 )
            if( get_obj_index( arg2 ) == NULL && fBootDb )
               boot_log( "%s: SmaugWiz - %s (%d) 'H': object %d no existe.", __FUNCTION__, room->area->filename, count,
                         arg2 );
         break;
...
void load_room_reset( ROOM_INDEX_DATA * room, FILE * fp )
{
   ...
   switch ( letter )
   {
      default:
         bug( "%s: bad command '%c'.", __FUNCTION__, letter );
         if( fBootDb )
            boot_log( "%s: %s (%d) comando equivocado '%c'.", __FUNCTION__, room->area->filename, count, letter );
         return;

      case 'M':
         if( get_mob_index( arg1 ) == NULL && fBootDb )
            boot_log( "%s: %s (%d) 'M': mobile %d no existe.", __FUNCTION__, room->area->filename, count, arg1 );
         break;

      case 'O':
         if( get_obj_index( arg1 ) == NULL && fBootDb )
            boot_log( "%s: %s (%d) '%c': objeto %d no existe.", __FUNCTION__, room->area->filename, count, letter,
                      arg1 );
         break;
      ...
      case 'P':
         ...
         if( arg3 <= 0 )
            arg3 = OBJ_VNUM_MONEY_ONE; /* This may look stupid, but for some reason it works. */
         if( get_obj_index( arg3 ) == NULL && fBootDb )
            boot_log( "%s: %s (%d) 'P': el objeto destino %d no existe.", __FUNCTION__, room->area->filename, count, arg3 );
         if( extra > 1 )
            not01 = TRUE;
         break;
      ...
      case 'E':
         if( get_obj_index( arg1 ) == NULL && fBootDb )
            boot_log( "%s: %s (%d) '%c': objeto %d no existe.", __FUNCTION__, room->area->filename, count, letter, arg1 );
         break;
      ...
      case 'D':
             ...
             || !( pexit = get_exit( room, arg2 ) ) || !IS_SET( pexit->exit_info, EX_ISDOOR ) )
         {
            ...
            bug( "Reset: %c %d %d %d %d", letter, extra, arg1, arg2, arg3 );
            if( fBootDb )
               boot_log( "%s: %s (%d) 'D': salida %d sin puerta.", __FUNCTION__, room->area->filename, count, arg2 );
         }
      ...
      case 'R':
         if( arg2 < 0 || arg2 > 10 )
         {
            bug( "%s: 'R': bad exit %d.", __FUNCTION__, arg2 );
            if( fBootDb )
               boot_log( "%s: %s (%d) 'R': %d no es una salida.", __FUNCTION__, room->area->filename, count, arg2 );
            break;
         }
         break;
   }
...
void load_rooms( AREA_DATA * tarea, FILE * fp )
{
   ...
   for( ;; )
   {
      ...
      if( get_room_index( vnum ) != NULL )
      {
         if( tmpBootDb )
         {
            bug( "Cargando habitaciones: vnum %d duplicada.", vnum );
            shutdown_mud( "duplicate vnum" );
            exit( 1 );
         }
         else
         {
            ...
            log_printf_plus( LOG_BUILD, sysdata.log_level, "Limpiando habitacion: %d", vnum );
      ...
      for( ;; )
      {
         ...
         if( letter == 'D' )
         {
            ...
            if( door < 0 || door > 10 )
            {
               bug( "Fread_rooms: vnum %d tiene un numero de puerta equivocado %d.", vnum, door );
               if( fBootDb )
                  exit( 1 );
            }
            else
...
void load_specials( FILE * fp )
{
   for( ;; )
   {
      ...
      {
         default:
            bug( "Cargando datos especiales: la letra '%c' no es *MS.", letter );
            exit( 1 );
         ...
         case 'M':
         {
            ...
            if( !pMobIndex )
            {
               bug( "%s: 'M': mob vnum incorrecto!", __FUNCTION__ );
               break;
            }
...
void load_ranges( AREA_DATA * tarea, FILE * fp )
{
   ...
   if( !tarea )
   {
      bug( "Cargando rangos de nivel: todavia no se ha encontrado #AREA." );
      shutdown_mud( "No #AREA" );
      exit( 1 );
   }
   ...
   for( iHash = 0; iHash < MAX_KEY_HASH; iHash++ )
   {
      for( pRoomIndex = room_index_hash[iHash]; pRoomIndex; pRoomIndex = pRoomIndex->next )
      {
         ...
         for( pexit = pRoomIndex->first_exit; pexit; pexit = pexit_next )
         {
            ...
            {
               if( fBootDb )
                  boot_log( "Establecer salidas: habitacion %d, la salida %s apunta a un vnum incorrecto (%d)",
                            pRoomIndex->vnum, dir_name[pexit->vdir], pexit->vnum );

               bug( "Eliminando la salida %s en la habitacion %d", dir_name[pexit->vdir], pRoomIndex->vnum );
   ...
   for( pArea = first_area; pArea; pArea = pArea->next )
   {
      ...
      if( pArea->nplayer > 0 && pArea->age == ( reset_age - 1 ) )
      {
         ...
         if( pArea->resetmsg )
            ...
            mudstrlcpy( buf, "Oyes algunos sonidos tintineantes ...\r\n", MAX_STRING_LENGTH );
...
void free_char( CHAR_DATA * ch )
{
   ...
   if( ch->desc )
      bug( "%s: el personaje aun tiene un descriptor.", __FUNCTION__ );
...
bool is_valid_filename( CHAR_DATA * ch, const char *direct, const char *filename )
{
   ...
   if( !filename || filename[0] == '\0' || strlen( filename ) < 3 )
   {
      if( !filename || !str_cmp( filename, "" ) )
         send_to_char( "El nombre de fichero no puede estar vacio.\r\n", ch );
      else
         ch_printf( ch, "%s: El nombre del fichero es demasiado corto.\r\n", filename );
      return FALSE;
   }
   ...
   {
      send_to_char( "El nombre de un fichero no debe contener '..', '/', or '\\' in it.\r\n", ch );
      return FALSE;
   }
   ...
   {
      ch_printf( ch, "%s el fichero ya existe.\r\n", newfilename );
      return FALSE;
   }
...
char *fread_flagstring( FILE * fp )
{
   ...
   for( ;; )
   {
      ...
      {
         bug( "%s: cadena demasiado larga", __FUNCTION__ );
         *plast = '\0';
         return flagstring;
      }
...
void do_memory( CHAR_DATA * ch, char *argument )
{
   ...
   send_to_char_color( "\r\n&wMemoria del sistema [argumentos - hash, check, showhigh]\r\n", ch );
   ch_printf_color( ch, "&wAfectos:   &W%5d\t\t\t&wZonas:     &W%5d\r\n", top_affect, top_area );
   ch_printf_color( ch, "&wExtDes:    &W%5d\t\t\t&wSalidas:   &W%5d\r\n", top_ed, top_exit );
   ch_printf_color( ch, "&wAyudas:    &W%5d\t\t\t&wReinicios: &W%5d\r\n", top_help, top_reset );
   ch_printf_color( ch, "&wIdxMobs:   &W%5d\t\t\t&wCriaturas: &W%5d\r\n", top_mob_index, nummobsloaded );
   ch_printf_color( ch, "&wIdxObjs:   &W%5d\t\t\t&wObjs:      &W%5d(%d)\r\n", top_obj_index, numobjsloaded, physicalobjects );
   ch_printf_color( ch, "&wHabitas:   &W%5d\t\t\t&wVHabit:    &W%5d\r\n", top_room, top_vroom );
   ch_printf_color( ch, "&wTiendas:   &W%5d\t\t\t&wArmerias:  &W%5d\r\n", top_shop, top_repair );
   ch_printf_color( ch, "&wObjetos:   &W%5d\t\t\t&wCurCq's:   &W%5d\r\n", cur_qobjs, cur_qchars );
   ch_printf_color( ch, "&wJugadores: &W%5d\t\t\t&wMaxJugrs:  &W%5d\r\n", num_descriptors, sysdata.maxplayers );
   ch_printf_color( ch, "&wMaxEver:   &W%5d\t\t\t&wHechizos:  &W%5d(%d)\r\n", sysdata.alltimemax, num_skills, MAX_SKILL );
   ch_printf_color( ch, "&wMaxEver ha sido en:  &W%s\r\n\r\n", sysdata.time_of_max );
   ch_printf_color( ch, "&wPocion Valor:  &W%-16d   &wScribe/Brew: &W%d/%d\r\n",
...
#else
      send_to_char( "La encriptacion de cadenas esta deshabilitada.\r\n", ch );
#endif
      return;
   }
...
#else
      send_to_char( "Encriptacion de cadenas no habilitada.\r\n", ch );
#endif
      return;
   }
   if( argument[0] != '\0' )
   ...
   {
#ifdef HASHSTR
      ch_printf( ch, "Estadisticas encriptacion:\r\n%s", hash_stats(  ) );
      if( hash != -1 )
         hash_dump( hash );
#else
      send_to_char( "Encriptacion de cadenas no habilitada.\r\n", ch );
...
void append_file( CHAR_DATA * ch, char *file, char *str )
{
   ...
   {
      perror( file );
      send_to_char( "No se ha podido abrir el fichero!\r\n", ch );
   }
   else
   {
...
void bug( const char *str, ... )
{
   ...
   mudstrlcpy( buf, "[DBBUG]: ", MAX_STRING_LENGTH );
   {
      va_list param;
   ...
   if( fpArea != NULL )
   {
      ...
      log_printf( "[DBBUG] FILE: %s LINE: %d", strArea, iLine );
      ...
      if( stat( SHUTDOWN_FILE, &fst ) != -1 )   /* file exists */
      {
         ...
         {
            fprintf( fp, "%s\n", buf );
            fprintf( fp, "[DBBUG] FICHERO: %s LINEA: %d\n", strArea, iLine );
            fclose( fp );
            fp = NULL;
         }
      }
...
void boot_log( const char *str, ... )
{
   ...
   mudstrlcpy( buf, "[DBLOG] ", MAX_STRING_LENGTH );
   va_start( param, str );
   vsnprintf( buf + strlen( buf ), ( MAX_STRING_LENGTH - strlen( buf ) ), str, param );
   va_end( param );
   log_string( buf );
...
#ifdef DEBUG
   log_string( "Agregar a listado de WIZ ..." );
#endif
...
void make_wizlist(  )
{
   ...
   unlink( WIZLIST_FILE );
   snprintf( buf, MAX_STRING_LENGTH, " Grandes Maestros de %s!", sysdata.mud_name );
   towizfile( buf );
   buf[0] = '\0';
   ilevel = 65535;
   for( wiz = first_wiz; wiz; wiz = wiz->next )
   {
      if( wiz->level < ilevel )
      {
         ...
         {
            case MAX_LEVEL - 0:
               towizfile( " El Maestro de las Tinieblas, Entidad Suprema" );
               break;
            case MAX_LEVEL - 1:
               towizfile( " Infinito" );
               break;
            ...
            case MAX_LEVEL - 3:
               ...
               break;
            case MAX_LEVEL - 4:
               towizfile( " Dioses Exaltados" );
               break;
            case MAX_LEVEL - 5:
               towizfile( " Dioses Ascendidos" );
               break;
            case MAX_LEVEL - 6:
               towizfile( " Grandes Dioses" );
               break;
            case MAX_LEVEL - 7:
               towizfile( " Dioses" );
               break;
            case MAX_LEVEL - 8:
               towizfile( " Dioses Menores" );
               break;
            case MAX_LEVEL - 9:
               towizfile( " Inmortales" );
               break;
            case MAX_LEVEL - 10:
               towizfile( " SemiDioses" );
               break;
            case MAX_LEVEL - 11:
               towizfile( " Guardianes" );
               break;
            case MAX_LEVEL - 12:
               towizfile( " Creadores" );
               break;
...
void mobprog_file_read( MOB_INDEX_DATA * mob, char *f )
{
   ...
   {
      bug( "%s: No se ha podido abrir el mudprog ", __FUNCTION__ );
      return;
   }
   ...
   for( ;; )
   {
      ...
      {
         case ERROR_PROG:
            bug( "%s: error en el tipo de fichero de mudprog", __FUNCTION__ );
            DISPOSE( mprg );
            continue;
...
void objprog_file_read( OBJ_INDEX_DATA * obj, char *f )
{
   ...
   if( !( progfile = fopen( MUDProgfile, "r" ) ) )
   {
      bug( "%s: no se puede abrir mudprog", __FUNCTION__ );
      return;
   }
...
OBJ_INDEX_DATA *make_object( int vnum, int cvnum, char *name )
{
   ...
   if( !cObjIndex )
   {
      snprintf( buf, MAX_STRING_LENGTH, "Se ha creado un %s", name );
      pObjIndex->short_descr = STRALLOC( buf );
      snprintf( buf, MAX_STRING_LENGTH, "Algun dios ha dejado algo recien creado %s aqui.", name );
...
MOB_INDEX_DATA *make_mobile( int vnum, int cvnum, char *name )
{
   ...
   if( !cMobIndex )
   {
      snprintf( buf, MAX_STRING_LENGTH, "Se ha creado un nuevo %s", name );
      pMobIndex->short_descr = STRALLOC( buf );
      snprintf( buf, MAX_STRING_LENGTH, "Algun dios ha abandonado un crecien creado %s aqui.\r\n", name );
...
void process_sorting( AREA_DATA * tarea )
{
   ...
   fprintf( stderr, "%-23s: Habitaciones: %5d - %-5d Objetos: %5d - %-5d Criaturas: %5d - %d\n",
            tarea->filename,
...
EXTRA_DESCR_DATA *fread_fuss_exdesc( FILE * fp )
{
   ...
   for( ;; )
   {
      ...
      if( word[0] == '\0' )
      {
         log_printf( "%s: EOF encontrado leyendo el fichero!", __FUNCTION__ );
         word = "#ENDEXDESC";
      }
      ...
      {
         default:
            log_printf( "%s: no coincide: %s", __FUNCTION__, word );
            fread_to_eol( fp );
            break;
         ...
         case '#':
            if( !str_cmp( word, "#ENDEXDESC" ) )
            {
               if( !ed->keyword )
               {
                  bug( "%s: La clave ExDesc no se ha encontrado. Devuelve NULL.", __FUNCTION__ );
                  STRFREE( ed->description );
                  DISPOSE( ed );
                  return NULL;
               }
      ...
      switch ( word[0] )
      {
         ...
         case 'D':
            ...
            if( !str_cmp( word, "Direction" ) )
            {
               ...
               {
                  bug( "%s: vnum %d tiene un numero de puerta incorrecto %d.", __FUNCTION__, pRoomIndex->vnum, door );
                  if( fBootDb )
                     return;
               }
               pexit = make_exit( pRoomIndex, NULL, door );
      ...
      switch ( word[0] )
      {
         ...
         case 'B':
            if( !str_cmp( word, "Bodyparts" ) )
            {
               ...
               while( bodyparts[0] != '\0' )
               {
                  ...
                  if( value < 0 || value > 31 )
                     bug( "Parte del cuerpo desconocida: %s", flag );
                  else
         ...
         case 'C':
            if( !str_cmp( word, "Class" ) )
            {
               ...
               {
                  bug( "%s: vnum %d: La criatura tiene una clase invalida! Se asignara Guerrero por defecto.", __FUNCTION__, pMobIndex->vnum );
         ...
         case 'D':
            ...
            if( !str_cmp( word, "DefPos" ) )
            {
               ...
               {
                  bug( "%s: vnum %d: La criatura tiene una posicion invalida! Estableciendo a esperar por defecto.", __FUNCTION__,
                       pMobIndex->vnum );
         ...
         case 'G':
            if( !str_cmp( word, "Gender" ) )
            {
               ...
               {
                  bug( "%s: vnum %d: La Criatura tiene un sexo invalido! Establecer a neutro por defecto.", __FUNCTION__, pMobIndex->vnum );
                  sex = SEX_NEUTRAL;
               }
               pMobIndex->sex = sex;
               break;
         ...
         case 'R':
            if( !str_cmp( word, "Race" ) )
            {
               ...
               {
                  bug( "%s: vnum %d: La criatura tiene una raza invalida! Estableciendo monstruo.", __FUNCTION__, pMobIndex->vnum );
...
void load_area_file( AREA_DATA * tarea, const char *filename )
{
   ...
   // Only seen at this stage for ayuda.zona
   ...
   for( ;; )
   {
      ...
      {
         bug( "%s: nombre de seccion incorrecto: %s", __FUNCTION__, word );
         if( fBootDb )
            exit( 1 );
         else
         {
...
void load_reserved( void )
{
   ...
   for( ;; )
   {
      if( feof( fp ) )
      {
         bug( "Cartando reservados: $ no encontrado." );
         fclose( fp );
         return;
      }
...
void load_buildlist( void )
{
   ...
   while( dentry )
   {
      if( dentry->d_name[0] != '.' )
      {
         ...
         if( rlow && rhi && !badfile )
         {
            snprintf( buf, MAX_STRING_LENGTH, "%s%s.zona", BUILD_DIR, dentry->d_name );
            if( !( fp = fopen( buf, "r" ) ) )
            {
               bug( "%s: no se puede leer el fichero de la zona", __FUNCTION__ );
               perror( buf );
               dentry = readdir( dp );
               continue;
            }
            ...
            {
               snprintf( buf, MAX_STRING_LENGTH, "%s: %s.zona: #AREA no encontrado.", __FUNCTION__, dentry->d_name );
               fclose( fp );
               fp = NULL;
               dentry = readdir( dp );
               continue;
...
#endif
            CREATE( pArea, AREA_DATA, 1 );
            snprintf( buf, MAX_STRING_LENGTH, "%s.zona", dentry->d_name );
            pArea->author = STRALLOC( dentry->d_name );
            pArea->filename = str_dup( buf );
#if !defined(READ_AREA)
            pArea->name = fread_string_nohash( fp );
#else
            snprintf( buf, MAX_STRING_LENGTH, "{PROTO} %s se esta construyendo esta zona", dentry->d_name );
...
void show_vnums( CHAR_DATA * ch, int low, int high, bool proto, bool shownl, char *loadst, char *notloadst )
{
   ...
   for( pArea = first_sort; pArea; pArea = pArea->next_sort )
   {
      ...
      if( IS_SET( pArea->status, AREA_LOADED ) )
         ...
         continue;
      pager_printf( ch, "%-15s| Habitaciones: %5d - %-5d"
                    " Objetos: %5d - %-5d Criaturas: %5d - %-5d%s\r\n",
   ...
   pager_printf( ch, "Zonas Listadas: %d  Cargadas: %d\r\n", count, loaded );
   return;
}
...
void save_sysdata( SYSTEM_DATA sys )
{
   ...
   snprintf( filename, MAX_INPUT_LENGTH, "%ssistema.datos", SYSTEM_DIR );
...
void do_check_vnums( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg1[0] == '\0' )
   {
      send_to_char( "Por favor, especifica una habitacion (room), criatura (mob), objeto (object) o todo (all) como tu primer parametro.\r\n", ch );
      return;
   }
   ...
   for( pArea = first_bsort; pArea; pArea = pArea->next_sort )
   {
      ...
      if( area_conflict )
      {
         ch_printf( ch, "Conflicto:%-15s| ", ( pArea->filename ? pArea->filename : "(invalid)" ) );
         if( room )
            ch_printf( ch, "Habitaciones: %5d - %-5d\r\n", pArea->low_r_vnum, pArea->hi_r_vnum );
         if( mob )
            ch_printf( ch, "Criaturas: %5d - %-5d\r\n", pArea->low_m_vnum, pArea->hi_m_vnum );
         if( obj )
            ch_printf( ch, "Objetos: %5d - %-5d\r\n", pArea->low_o_vnum, pArea->hi_o_vnum );
      }
   }
}
